...
# ---------------------------------------------------------------------------
# Transforms — separate train vs val/test pipelines
# ---------------------------------------------------------------------------
...
def build_train_transform(image_size: int = 224) -> T.Compose:
    ...
    Augmentations simulate realistic document/image variability so the
    model cannot rely on dataset-level shortcuts:
      - RandomResizedCrop : handles framing / scanning differences
      - Flips             : documents can be scanned either orientation
      - ColorJitter       : counters scanner colour variation
      - RandomRotation    : small tilts from scanning / photographing
      - RandomErasing     : simulates stamps, redactions, or stickers
    """
    return T.Compose([
...
def build_model(
    ...
    """
    ResNet-18 with Dropout + Linear head for binary fraud detection.

    Two-stage training strategy:
      Stage 1 — freeze_backbone=True : only head trains; protects pretrained
                 features from noisy early gradients (catastrophic forgetting).
...
def tune_threshold(
    ...
    metric="f1"     -> balanced precision/recall trade-off
    metric="recall" -> maximise fraud detection (fewer missed frauds)
    """
...
def subgroup_metrics(
    ...
    Primary diagnostic for dataset-level shortcuts: if the model has
    near-perfect recall on one source_dataset and poor recall on another,
    it learned dataset artefacts rather than genuine fraud cues.
    """
    work = df.copy()
...
class GradCAM:
    """
    Lightweight GradCAM for ResNet-style models.

    Usage
    -----
    cam     = GradCAM(model, target_layer=model.layer4[-1])
    heatmap = cam(image_tensor.unsqueeze(0).to(device))
    overlay = GradCAM.overlay(pil_image, heatmap)

    If the heatmap lights up document backgrounds / borders rather than
    the signature or stamp, the model learned a shortcut and needs
    stronger augmentation or background masking.
    """
