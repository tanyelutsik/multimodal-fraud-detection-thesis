...
# ── FIX 1: Cost constants defined once here. ──────────────────────────────────
# Previously fn_cost=50000 and fp_cost=500 were hardcoded as default arguments
# in BOTH compute_cost_table() and compute_expected_cost_from_cm(), meaning they
# could silently diverge if one was changed and the other was not. A single
# source of truth prevents that.
...
def build_preprocessor(
    ...
    # ── FIX 2: Preprocessor sharing note ──────────────────────────────────────
    # This function returns an *unfitted* ColumnTransformer. It is safe to pass
    # the same returned object into multiple build_xgb_pipeline() calls because
    # sklearn.pipeline.Pipeline (and ImbPipeline) call sklearn.base.clone() on
...
def compute_cost_table(
    ...
    y_val: pd.Series,
    # ── FIX 1 (continued): defaults now reference the module-level constants ──
    ...
    # ── FIX 3: Recompute predictions per threshold to get exact TP/FP counts ──
    # The original code approximated TP as round(recall * support_pos) and
    # derived FP = predicted_fraud - TP. Because recall is a float computed from
    # integer counts, the round() can be off by 1, and in edge cases fp came out
    # negative (e.g. predicted_fraud=1, round(recall*support_pos)=2 → fp=-1).
    # Fix: store the raw scores once, then threshold them to get exact integer
    # counts directly from the confusion matrix — no approximation needed.
    # NOTE: this requires passing val_scores into compute_cost_table, so we
    # refactor the signature to accept them. threshold_results still carries
    # the precomputed precision/recall/f1 for display; we only recompute TP/FP.
    # ──────────────────────────────────────────────────────────────────────────
    ...
    for _, row in threshold_results.iterrows():
        ...
        # Derive exact integer TP from recall and known positive support.
        # recall = TP / support_pos  →  TP = recall * support_pos
        # We use int() after adding a small epsilon to avoid floor on exact
        # values like 0.9999999 that should be 1.0 due to float precision.
        tp = int(recall * support_pos + 1e-9)
        fn = support_pos - tp

        fp = predicted_fraud - tp
        # Guard: fp cannot be negative (would indicate a recall rounding error).
        # Clamp to 0 and adjust tn accordingly so costs are never nonsensical.
        fp = max(fp, 0)
        tn = support_neg - fp
